...
#
# Modified by Jaakko Kivioja, 2025
# 
# Alkuperäistä koodia muutettu niin että ohjelma käyttää sanakirjoja listojen sijaan.
# Sanakirjaa käyttämällä koodista tulee luettavampi vrt. varaus[1] vs. varaus["nimi"].
# 
# See LICENSE file in the project root for full license information.


'''Ohjelma lukee varaukset tiedostosta ja tulostaa erilaisia tietoja niistä konsoliin.'''

from datetime import datetime

def muunna_varaustiedot(varaus: list) -> dict:
    '''Muuntaa tiedot oikeaan muotoon ja palauttaa sanakirjana.'''
    ...
    return {
        ...
        "vahvistus": varaus[8].lower() == "true",
        "tila": varaus[9],
        "luotu": datetime.strptime(varaus[10], "%Y-%m-%d %H:%M:%S")
    }

def hae_varaukset(varaustiedosto: str) -> list:
    '''Hakee varaukset tiedostosta ja palauttaa listan sanakirjoja.'''
...
def vahvistetut_varaukset(varaukset: list):
    '''Hakee ja tulostaa vahvistetut varaukset.'''
...
def pitkat_varaukset(varaukset: list):
    '''Hakee ja tulostaa yli 3 tuntia kestävät varaukset.'''
    for varaus in varaukset:
        if(varaus["kesto"] >= 3):
...
def varausten_lkm(varaukset: list):
    '''Laskee ja tulostaa vahvistettujen ja ei-vahvistettujen varausten lukumäärän.'''
...
def main():
    '''Pääohjelma, joka kutsuu muita funktioita ja tulostaa tiedot konsoliin.'''
